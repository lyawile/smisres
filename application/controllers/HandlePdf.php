...
class HandlePdf extends CI_Controller {
    ...
    public function show() {
        ...
        $class = 1;
        $test = $this->result_model->getResults($class);
        foreach ($test->result() as $d) {
            $this->pdf->AddPage();
            $this->pdf->SetFont('Arial', 'B', 15);
//        }
            $this->pdf->Cell(189, 10, "KWA MZAZI / MLEZI WA: ". strtoupper($d->studNames), 1, 1, 'C');
...
//        $this->pdf->Cell(20, 7.5, "WASTANI",1);
            ...
            for ($i = 1; $i <= 1; $i++) {
                ...
                $this->pdf->Cell(40, 5, "Somo", 1, '', "C");
                $this->pdf->Cell(20, 5, "$d->march", 1, '', "C");
                $this->pdf->Cell(20, 5, "$d->june", 1, '', "C");
                $this->pdf->Cell(20, 5, "$d->avgJune", 1, '', "C");
